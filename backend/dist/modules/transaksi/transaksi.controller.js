...
export class TransaksiController {
    ...
    async exportPdf(request, reply) {
        ...
        transaksi.forEach((t, idx) => {
            if (idx % 2 === 0)
                doc.rect(PAGE_MARGIN, ry, CW, rowH).fill(LIGHT_BG);
            ...
            doc.fillColor(DARK).font('Helvetica').fontSize(9).text(tgl, rx, ry + 6, { width: cols[0] - 8 });
            rx += cols[0];
            doc.text(t.deskripsi || '-', rx, ry + 6, { width: cols[1] - 8 });
            rx += cols[1];
