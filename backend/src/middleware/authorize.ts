...
// ──────────────────────────────────────────────────────────
// Izin format: resource:action
// Contoh: "booking:baca", "booking:ubah", "*:baca"
//
// * di role = semua izin
// ──────────────────────────────────────────────────────────
...
export function punyaIzin(role: Role, izin: Permission): boolean {
  const rolePermissions = hakAkses[role];
  if (!rolePermissions) return false;
  if (rolePermissions.includes('*')) return true;

  // Cek exact match
  if (rolePermissions.includes(izin)) return true;

  // Cek "resource:*" wildcard (e.g., "booking:*" = semua aksi booking)
  const [resource] = izin.split(':');
  if (resource && rolePermissions.includes(`${resource}:*`)) return true;
  ...
  return async function (request: FastifyRequest, reply: FastifyReply) {
    ...
    if (missingPermissions.length > 0) {
      throw new ForbiddenError(
        `Kamu tidak punya izin: ${missingPermissions.join(', ')}`
      );
    }
  };
}
